const EVALUATIONS = [
  {order:0, code:'data0', label:'Recopilación Examen 2026-II', short:'Recop.', path:'data0/resultados.xlsx?v=9', color:'#755282', text:'#FFFFFF'},
  {order:1, code:'data1', label:'Simulacro I', short:'Sim. I', path:'data1/resultados.xlsx?v=9', color:'#B9964A', text:'#FFFFFF'},
  {order:2, code:'data2', label:'Simulacro II', short:'Sim. II', path:'data2/resultados.xlsx?v=9', color:'#617F9D', text:'#FFFFFF'},
  {order:3, code:'data3', label:'Simulacro III', short:'Sim. III', path:'data3/resultados.xlsx?v=9', color:'#A56868', text:'#FFFFFF'},
  {order:4, code:'data4', label:'Simulacro IV', short:'Sim. IV', path:'data4/resultados.xlsx?v=9', color:'#688A72', text:'#FFFFFF'}
];
...
const ADMIN_ID = 'PROYECTIA';
const TEACHER_ID = 'AREA';
const DEMO_ID = '1234';
const STORE = new Map();
let currentDoc = '';
let currentRecords = [];
...
let teacherEvaluationOrder = 1;
let teacherArea = AREA_CONFIG[0].name;
...
function showOnly(id){
  ['#menu-view','#report-view','#progress-view','#admin-view','#teacher-view'].forEach(x=>$(x).classList.toggle('hidden',x!==id));
...
function isTeacherAccess(value){
  return String(value ?? '')
    .trim()
    .replace(/\s+/g,'')
    .toUpperCase() === TEACHER_ID;
}
function isRealStudentDoc(value){
  const doc=normDoc(value);
  return !!doc && doc!==DEMO_ID;
}

function searchStudent(){
  const rawValue=$('#documento').value;
  const estado=$('#estado');

  // La vista por áreas se intercepta antes de cualquier búsqueda de estudiante.
  if(isTeacherAccess(rawValue)){
    estado.textContent='';
    currentDoc='';
    currentRecords=[];
    renderTeacher();
  ...
  // El acceso maestro se intercepta antes de cualquier búsqueda de estudiante.
  if(isAdminAccess(rawValue)){
    estado.textContent='';
    currentDoc='';
    currentRecords=[];
    renderAdmin();
  ...
  if(!found.length){
    ...
    ['#menu-view','#report-view','#progress-view','#admin-view','#teacher-view'].forEach(x=>$(x).classList.add('hidden'));
    return;
  }
  currentDoc=doc; currentRecords=found; estado.textContent=''; renderMenu(found);
}

function recordsForDoc(doc){
  const found=[];
  for(const ev of EVALUATIONS){
    const row=resultFor(ev.order,doc);
    if(row) found.push({ev,row,data:STORE.get(ev.order)});
  }
  return found;
}

function collectAdminStudents(){
  const byDoc=new Map();
  for(const ev of EVALUATIONS){
    const data=STORE.get(ev.order);
    for(const row of (data?.resultados||[])){
      const doc=normDoc(row.documento);
      if(!doc) continue;
      if(!byDoc.has(doc)) byDoc.set(doc,{documento:doc,nombre:'',scores:new Map(),count:0});
      const item=byDoc.get(doc);
      const name=getName(row);
      if(name) item.nombre=name;
      const score=globalValue(row.puntaje_global);
      if(score!==null) item.scores.set(ev.order,score);
      item.count=item.scores.size;
    }
  }
  return [...byDoc.values()].sort((a,b)=>{
    const an=(a.nombre||'').toLocaleLowerCase('es');
    const bn=(b.nombre||'').toLocaleLowerCase('es');
    return an.localeCompare(bn,'es') || a.documento.localeCompare(b.documento,'es', {numeric:true});
  });
}

function adminScoreCell(student,ev){
  const score=student.scores.get(ev.order);
  return score===undefined?'—':score.toLocaleString('es-CO');
}

function renderAdmin(filter=''){
  const all=collectAdminStudents();
  const q=String(filter||'').trim().toLocaleLowerCase('es');
  const students=q ? all.filter(s=>(s.nombre||'').toLocaleLowerCase('es').includes(q)||s.documento.toLocaleLowerCase('es').includes(q)) : all;
  const counts=EVALUATIONS.map(ev=>({ev,n:(STORE.get(ev.order)?.resultados||[]).filter(r=>normDoc(r.documento)).length}));
  $('#admin-view').innerHTML=`
    <div class="admin-toolbar">
      <div>
        <span class="admin-kicker">Acceso administrativo</span>
        <h2>Resultados de estudiantes</h2>
        <p>${all.length} estudiantes con al menos un resultado cargado.</p>
      </div>
      <button class="btn-small" onclick="closeAdmin()">Volver a la consulta</button>
    </div>
    <section class="admin-stats">
      ${counts.map(x=>`<article style="--eval-color:${x.ev.color}"><span>${esc(x.ev.short)}</span><strong>${x.n}</strong><small>resultados</small></article>`).join('')}
    </section>
    <section class="card wide admin-panel">
      <div class="admin-panel-head">
        <div><h2>Todos los estudiantes</h2><p>Selecciona un estudiante para abrir su reporte completo.</p></div>
        <label class="admin-search"><span class="sr-only">Buscar estudiante</span><input id="admin-filter" type="search" value="${esc(filter)}" placeholder="Buscar por nombre o documento" oninput="filterAdmin(this.value)"></label>
      </div>
      <div class="table-wrap admin-table-wrap">
        <table class="responsive-table admin-table">
          <thead><tr><th>Estudiante</th><th>Documento</th>${EVALUATIONS.map(ev=>`<th>${esc(ev.short)}</th>`).join('')}<th></th></tr></thead>
          <tbody>${students.map(s=>`<tr>
            <td data-label="Estudiante"><strong class="admin-student-name">${esc(s.nombre||'Sin nombre registrado')}</strong></td>
            <td data-label="Documento">${esc(s.documento)}</td>
            ${EVALUATIONS.map(ev=>`<td data-label="${esc(ev.short)}" class="admin-score">${adminScoreCell(s,ev)}</td>`).join('')}
            <td data-label="Abrir"><button class="admin-open" onclick="openStudentFromAdmin('${esc(s.documento)}')">Ver resultados</button></td>
          </tr>`).join('')}</tbody>
        </table>
      </div>
      ${students.length?'':`<p class="admin-empty">No hay estudiantes que coincidan con la búsqueda.</p>`}
    </section>`;
  showOnly('#admin-view');
  const input=$('#admin-filter');
...
function filterAdmin(value){
  clearTimeout(adminFilterTimer);
  adminFilterTimer=setTimeout(()=>renderAdmin(value),90);
}

function openStudentFromAdmin(doc){
  const found=recordsForDoc(normDoc(doc));
  if(!found.length) return;
  currentDoc=normDoc(doc);
  currentRecords=found;
  $('#documento').value=currentDoc;
  renderMenu(found);
}
...
function closeAdmin(){
  ...
  $('#documento').focus({preventScroll:true});
}


function teacherAvailableEvaluations(){
  return EVALUATIONS.filter(ev=>{
    const data=STORE.get(ev.order);
    return (data?.preguntas||[]).some(r=>isRealStudentDoc(r.documento));
  });
}
function teacherQuestions(order,area=''){
  const data=STORE.get(order);
  return (data?.preguntas||[]).filter(r=>isRealStudentDoc(r.documento) && (!area || String(r.area||'').trim()===area));
}
function teacherResults(order){
  const data=STORE.get(order);
  return (data?.resultados||[]).filter(r=>isRealStudentDoc(r.documento));
}
function isCorrectAnswerRow(r){
  const v=String(r?.resultado||'').trim().toLocaleLowerCase('es');
  return v==='correcta' || v==='correcto' || v==='correct';
}
function pctLabel(v){
  const n=num(v);
  return n===null?'—':`${Math.round(n)}%`;
}
function teacherAreaStats(order,area){
  const qs=teacherQuestions(order,area);
  const docs=new Set(qs.map(r=>normDoc(r.documento)).filter(Boolean));
  const items=new Set(qs.map(r=>String(r.pregunta)).filter(Boolean));
  const correct=qs.filter(isCorrectAnswerRow).length;
  const accuracy=qs.length ? correct*100/qs.length : null;
  const cfg=AREA_CONFIG.find(a=>a.name===area);
  const scores=teacherResults(order).map(r=>cfg?num(r[cfg.score]):null).filter(v=>v!==null);
  const avgScore=scores.length?scores.reduce((a,b)=>a+b,0)/scores.length:null;
  return {qs,students:docs.size,items:items.size,correct,total:qs.length,accuracy,avgScore};
}
function teacherAreaCards(order){
  return AREA_CONFIG.map(a=>{
    const st=teacherAreaStats(order,a.name);
    return `<button class="teacher-area-card ${teacherArea===a.name?'selected':''}" data-area="${esc(a.name)}" aria-pressed="${teacherArea===a.name?'true':'false'}" onclick="showTeacherArea('${a.name.replace(/'/g,"\\'")}')">
      <span>${esc(a.name)}</span>
      <strong>${st.accuracy===null?'—':pctLabel(st.accuracy)}</strong>
      <small>acierto general</small>
    </button>`;
  }).join('');
}
function teacherTopicStats(order,area){
  const grouped=new Map();
  for(const r of teacherQuestions(order,area)){
    const tema=String(r.tema||'').trim() || 'Sin tema registrado';
    const sub=String(r.subarea||'').trim();
    const key=`${sub}||${tema}`;
    if(!grouped.has(key)) grouped.set(key,{tema,subarea:sub,correct:0,total:0});
    const g=grouped.get(key); g.total++; if(isCorrectAnswerRow(r)) g.correct++;
  }
  return [...grouped.values()].map(x=>({...x,pct:x.total?x.correct*100/x.total:0}));
}
function teacherTopicLabel(t,area){
  if(area==='Ciencias naturales' && t.subarea) return `${esc(t.subarea)} · ${esc(t.tema)}`;
  return esc(t.tema);
}
function teacherTopicLists(order,area){
  const rows=teacherTopicStats(order,area);
  if(!rows.length) return `<p class="teacher-empty">No hay información temática disponible para esta prueba.</p>`;
  const top=[...rows].sort((a,b)=>b.pct-a.pct || b.total-a.total).slice(0,4);
  const topKeys=new Set(top.map(x=>`${x.subarea}||${x.tema}`));
  let low=[...rows].sort((a,b)=>a.pct-b.pct || b.total-a.total).filter(x=>!topKeys.has(`${x.subarea}||${x.tema}`)).slice(0,4);
  if(!low.length) low=[...rows].sort((a,b)=>a.pct-b.pct || b.total-a.total).slice(0,Math.min(4,rows.length));
  const list=items=>`<ul>${items.map(t=>`<li><span>${teacherTopicLabel(t,area)}</span><strong>${pctLabel(t.pct)}</strong></li>`).join('')}</ul>`;
  return `<div class="teacher-topic-grid">
    <section><h3>Temas con mayor porcentaje de acierto</h3>${list(top)}</section>
    <section><h3>Temas para revisar con mayor atención</h3>${list(low)}</section>
  </div>`;
}
function teacherNaturalBreakdown(order,area){
  if(area!=='Ciencias naturales') return '';
  const rows=teacherQuestions(order,area);
  const grouped=new Map();
  for(const r of rows){
    const sub=String(r.subarea||'').trim(); if(!sub) continue;
    if(!grouped.has(sub)) grouped.set(sub,{correct:0,total:0});
    const g=grouped.get(sub); g.total++; if(isCorrectAnswerRow(r)) g.correct++;
  }
  if(!grouped.size) return '';
  return `<div class="teacher-subarea-strip">${[...grouped.entries()].map(([name,g])=>{
    const p=g.total?g.correct*100/g.total:0;
    return `<div><span>${esc(name)}</span><strong>${pctLabel(p)}</strong><i><b style="width:${Math.max(0,Math.min(100,p))}%"></b></i></div>`;
  }).join('')}</div>`;
}
function teacherItemStats(order,area){
  const grouped=new Map();
  for(const r of teacherQuestions(order,area)){
    const q=String(r.pregunta||'').trim(); if(!q) continue;
    if(!grouped.has(q)) grouped.set(q,{pregunta:q,subarea:String(r.subarea||'').trim(),tema:String(r.tema||'').trim(),correcta:String(r.respuesta_correcta||'').trim(),correct:0,total:0});
    const g=grouped.get(q); g.total++; if(isCorrectAnswerRow(r)) g.correct++;
    if(!g.correcta && r.respuesta_correcta) g.correcta=String(r.respuesta_correcta).trim();
  }
  return [...grouped.values()].map(x=>({...x,pct:x.total?x.correct*100/x.total:0})).sort((a,b)=>{
    const an=Number(a.pregunta),bn=Number(b.pregunta);
    return (Number.isFinite(an)&&Number.isFinite(bn))?an-bn:String(a.pregunta).localeCompare(String(b.pregunta),'es',{numeric:true});
  });
}
function teacherItemTable(order,area){
  const rows=teacherItemStats(order,area);
  if(!rows.length) return `<p class="teacher-empty">No hay respuestas disponibles para esta área.</p>`;
  return `<div class="teacher-item-window table-wrap"><table class="responsive-table teacher-item-table">
    <thead><tr><th>Pregunta</th><th>${area==='Ciencias naturales'?'Subárea':'Tema'}</th>${area==='Ciencias naturales'?'<th>Tema</th>':''}<th>Respuesta correcta</th><th>Porcentaje de acierto</th></tr></thead>
    <tbody>${rows.map(r=>`<tr>
      <td data-label="Pregunta"><strong>${esc(r.pregunta)}</strong></td>
      <td data-label="${area==='Ciencias naturales'?'Subárea':'Tema'}">${esc(area==='Ciencias naturales'?(r.subarea||'—'):(r.tema||'—'))}</td>
      ${area==='Ciencias naturales'?`<td data-label="Tema">${esc(r.tema||'—')}</td>`:''}
      <td data-label="Respuesta correcta" class="teacher-answer">${esc(r.correcta||'—')}</td>
      <td data-label="Porcentaje de acierto" class="teacher-pct-cell"><div class="teacher-pct"><span><i style="width:${Math.max(0,Math.min(100,r.pct))}%"></i></span><strong>${pctLabel(r.pct)}</strong></div></td>
    </tr>`).join('')}</tbody>
  </table></div>`;
}
function teacherEvaluationSelector(available){
  const on=new Set(available.map(e=>e.order));
  return `<div class="teacher-test-selector">${EVALUATIONS.map(ev=>`<button ${on.has(ev.order)?`onclick="selectTeacherEvaluation(${ev.order})"`:'disabled'} class="teacher-test-choice ${teacherEvaluationOrder===ev.order?'selected':''}" style="--eval-color:${ev.color}" aria-pressed="${teacherEvaluationOrder===ev.order?'true':'false'}"><i></i><span>${esc(ev.label)}</span>${on.has(ev.order)?'':'<small>Sin resultados</small>'}</button>`).join('')}</div>`;
}
function renderTeacher(){
  const available=teacherAvailableEvaluations();
  if(!available.length){
    $('#teacher-view').innerHTML=`<div class="teacher-toolbar"><div><span class="teacher-kicker">Vista para profesores</span><h2>Rendimiento por áreas</h2><p>No hay pruebas con resultados de estudiantes disponibles.</p></div><button class="btn-small" onclick="closeTeacher()">Volver a la consulta</button></div>`;
    showOnly('#teacher-view'); return;
  }
  if(!available.some(e=>e.order===teacherEvaluationOrder)) teacherEvaluationOrder=available[available.length-1].order;
  const ev=evByOrder(teacherEvaluationOrder);
  $('#teacher-view').innerHTML=`
    <div class="teacher-toolbar" style="--eval-color:${ev.color}">
      <div><span class="teacher-kicker">Vista para profesores</span><h2>Rendimiento por áreas</h2><p>Selecciona una prueba y luego un área para revisar el desempeño general.</p></div>
      <button class="btn-small" onclick="closeTeacher()">Volver a la consulta</button>
    </div>
    <section class="card wide teacher-test-panel"><div class="section-title"><div><h2>Prueba</h2><span>Elige la prueba que quieres analizar.</span></div></div>${teacherEvaluationSelector(available)}</section>
    <section class="card wide"><div class="section-title"><div><h2>Áreas</h2><span>Selecciona una de las cinco áreas.</span></div></div><div id="teacher-area-cards" class="teacher-area-grid">${teacherAreaCards(teacherEvaluationOrder)}</div></section>
    <section id="teacher-area-detail"></section>`;
  showOnly('#teacher-view');
  showTeacherArea(teacherArea,true);
}
function selectTeacherEvaluation(order){
  teacherEvaluationOrder=order;
  teacherArea=AREA_CONFIG[0].name;
  renderTeacher();
}
function showTeacherArea(area,skipScroll=false){
  teacherArea=area;
  const ev=evByOrder(teacherEvaluationOrder);
  const stats=teacherAreaStats(teacherEvaluationOrder,area);
  const cards=$('#teacher-area-cards'); if(cards) cards.innerHTML=teacherAreaCards(teacherEvaluationOrder);
  const target=$('#teacher-area-detail'); if(!target) return;
  target.innerHTML=`
    <section class="card wide teacher-area-detail" style="--eval-color:${ev.color}">
      <div class="teacher-detail-head"><div><span class="teacher-current-test">${esc(ev.label)}</span><h2>${esc(area)}</h2></div></div>
      <div class="teacher-kpis">
        <article><span>Puntaje promedio del área</span><strong>${stats.avgScore===null?'—':stats.avgScore.toFixed(1)}</strong></article>
        <article><span>Porcentaje general de acierto</span><strong>${pctLabel(stats.accuracy)}</strong></article>
        <article><span>Estudiantes</span><strong>${stats.students}</strong></article>
        <article><span>Preguntas del área</span><strong>${stats.items}</strong></article>
      </div>
      ${teacherNaturalBreakdown(teacherEvaluationOrder,area)}
      <div class="teacher-topic-section"><h2>Rendimiento por temas</h2>${teacherTopicLists(teacherEvaluationOrder,area)}</div>
      <div class="teacher-items-section"><div class="section-title"><div><h2>Porcentaje de acierto por pregunta</h2><span>El porcentaje corresponde a los estudiantes que respondieron correctamente cada pregunta.</span></div></div>${teacherItemTable(teacherEvaluationOrder,area)}</div>
    </section>`;
  if(!skipScroll) target.scrollIntoView({behavior:'smooth',block:'start'});
}
function closeTeacher(){
  teacherEvaluationOrder=1;
  teacherArea=AREA_CONFIG[0].name;
  $('#documento').value='';
  ['#menu-view','#report-view','#progress-view','#admin-view','#teacher-view'].forEach(x=>$(x).classList.add('hidden'));
  window.scrollTo({top:0,behavior:'smooth'});
