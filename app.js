...
function showOnly(id){
  ...
  if(floating) floating.classList.toggle('hidden',!currentRecords.length || id!=='#report-view');
...
function scoreHistory(records){
  return `<div class="table-wrap score-history"><table><thead><tr><th>Prueba</th><th>Puntaje global</th><th>Diferencia frente a la prueba anterior presentada</th></tr></thead><tbody>${records.map((r,i)=>{const d=i?r.score-records[i-1].score:null;return `<tr><td><span class="table-eval-dot" style="background:${r.ev.color}"></span>${esc(r.ev.label)}</td><td><strong>${r.score}</strong></td><td>${d===null?'—':deltaGlobalHTML(d)}</td></tr>`}).join('')}</tbody></table></div>`;
}
...
function areaReading(records,area){
  ...
  return `<p>Estos son los puntajes registrados en ${esc(area)} para las pruebas que has presentado:</p><ul class="compact-score-list">${list}</ul>`;
}
function priorityTopicsForArea(area){
  ...
  for(const rec of currentRecords){
    ...
    for(const t of topics){
      ...
      if(!tema) continue;
      const subarea=String(t.subarea||'').trim();
      const hits=num(t.aciertos), total=num(t.total_preguntas);
      if(total===null || total<=0) continue;
      const key=`${subarea}||${tema}`;
      if(!grouped.has(key)) grouped.set(key,{tema,subarea,hits:0,total:0,tests:0});
  ...
  return `<div class="area-priority-block"><h4>Temas para estudiar primero</h4><p>Según tus respuestas en las pruebas que has presentado, conviene revisar primero:</p><ol class="area-priority-list">${rows.slice(0,4).map(t=>`<li><div><strong>${esc(t.tema)}</strong>${t.subarea?`<span>${esc(t.subarea)}</span>`:''}</div><b>${t.hits}/${t.total} correctas</b></li>`).join('')}</ol></div>`;
}
...
function renderAreaProgress(area){
  ...
  target.innerHTML=`<div class="area-progress-grid"><div><div class="area-mini-summary"><div class="mini-stat"><span>Puntaje más reciente</span><strong>${last.score.toFixed(1)}</strong></div><div class="mini-stat"><span>Diferencia entre el primero y el más reciente</span><strong>${vals.length>1?(d>0?'+':'')+d.toFixed(1):'—'}</strong></div><div class="mini-stat"><span>Mayor puntaje registrado</span><strong>${max.score.toFixed(1)}</strong></div></div>${areaChart(records,area)}</div><aside class="area-report"><h3>Resultados de ${esc(area)}</h3>${areaReading(records,area)}${priorityTopicsForArea(area)}</aside></div>`;
}

function progressReading(records){
  if(!records.length) return '';
  if(records.length===1) return `<p class="friendly-reading">Tienes un puntaje global registrado: <strong>${records[0].score}</strong> en ${esc(records[0].ev.label)}. Las demás pruebas aparecerán cuando exista información asociada a tu documento.</p>`;
